...
sig ActiveHeap extends Addr {}
sig RootSet in Object {}
sig Time {}

one sig SC {
	mem: Memory one -> Time,
	Copied: Object -> Time // set of addresses that have been copied?
} {
	mem.Time = Memory // No extraneous memories
}

fact AtMostOneMapping{
	all t: Time | all o: Object {
		lone a: ActiveHeap| a->o in SC.mem.t.data
	}
}
...
sig CopyEvent extends Event{} {
	...
	SC.mem.pre.data in SC.mem.post.data
